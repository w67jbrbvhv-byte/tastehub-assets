...
- **The real risk isn't paper money.** It is that a lucky run convinces you to
  ...
  Beating a buy-and-hold of the benchmark, net of costs, with no worse maximum
  drawdown, over at least twelve months, is a reasonable bar. Most strategies
  will not clear it.
...
On top of those: the `HALT` file. `python run.py halt` creates it and no order
...
recorded peak, all trading halts and stays halted until you intervene.
...
The risk engine tests are the ones that matter — they are what makes the claims
in the Safety section true rather than aspirational. If you change a limit's
behaviour, change the test first.
